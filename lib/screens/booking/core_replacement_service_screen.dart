...
class _CoreReplacementServiceScreenState
  ...
  void _createPayment() {
    final _amount = _totalPriced?.toString();
    if (_amount == null) {
      return;
    }
    //Môi trường Test
  ...
  Widget build(BuildContext context) {
    return BlocConsumer<CoreReplacementServiceBloc,
        CoreReplatementServiceState>(builder: _builder, listener: _listener);
  }
        ...
        child: Column(
          ...
          children: [
            ...
            Container(
                ...
                decoration: BoxDecoration(
                  border:
                      Border.all(color: const Color.fromRGBO(4, 107, 80, 1)),
                  borderRadius: BorderRadius.circular(10.0),
                ),
                child: _tableUpdate()),
            Wrap(
            ...
            if (_bloc.orderDetailModel?.paymentStatus == '1') ...[
              Container(
                  ...
                  child: ImageUtil.loadAssetsImage(
                      fileName: Images.successPayment, width: 150))
            ]
          ],
        ),
      ),
  ...
  Widget _showBottomSheetFeedback() {
    if (App.instance.userApp?.isUserCustomer() == true) {
      return SafeArea(
          bottom: true,
          child: Padding(
            padding: const EdgeInsets.only(bottom: 0.0, left: 8.0, right: 8.0),
            child: SizedBox(
              width: context.width,
              child: Row(
                ...
                children: [
                  ElevatedButton.icon(
                    ...
                    style: ButtonStyle(
                      backgroundColor:
                          MaterialStateProperty.all(ColorUtil.bangladeshGreen),
                    ),
                  ),
                  if (_bloc.orderDetailModel?.paymentStatus == '0')
                    ElevatedButton.icon(
                      ...
                      style: ButtonStyle(
                        backgroundColor:
                            MaterialStateProperty.all(ColorUtil.red),
                      ),
                    ...
                    onPressed: () {
                      Navigator.pushNamed(context, Routes.serviceScreen,
                          arguments: {
                            "listService": [],
                            "index": "",
                            "productId": _bloc.orderDetailModel
                                ?.orderFilterCoresModel?[0].orderDetailId
                          });
                    },
                    icon: const Icon(
                      Icons.add,
  ...
  Widget _tablePrice() {
    ...
    var cal = (Decimal.parse(totalPrice ?? "0") -
    ...
    return Table(
      children: [
        ...
        TableRow(children: [
          ...
          TableCell(
              child: Text(_totalPriced.toString().toVND(),
                  style: const TextStyle(
                      color: ColorUtil.brightYellow,
                      fontWeight: FontWeight.w600)))
        ]),
                ...
                itemBuilder: (BuildContext context, int index) {
                  return _buildItemMedia(
                      "$protocol${AppConfig.instance.values.apiUrl}/" +
                          _bloc.orderDetailModel!.images![index]);
                },
              ),
            ),
          )
                ...
                children: [
                  ...
                  _buildButtonDialog(
                      ...
                      action: () {
                        ...
                        _bloc.add(OrderFeedbackTaskProcessedEvent(
