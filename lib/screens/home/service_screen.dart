...
class _ServiceScreenState extends State<ServiceScreen> {
  ...
  Widget _buildCardServiceAndDevice() {
    return Container(
  ...
  Widget _buildDropdownFieldPruducts() {
    final hasProduct = _listProducts.any(
      (element) => element.id == _currentSelectedProductValue,
    );
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        DropdownButtonFormField<String>(
          ...
          onChanged: (String? newValue) {
            setState(() {
              ...
              var matched = _listProducts.where(
                (item) =>
                    item.id.toString() ==
                    _currentSelectedProductValue.toString(),
              );
              if (matched.isNotEmpty) {
                var add = matched.first.address ?? '';
