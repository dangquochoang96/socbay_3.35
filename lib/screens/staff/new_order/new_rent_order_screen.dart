...
class StaffNewRentOrderScreen extends StatefulWidget {
  const StaffNewRentOrderScreen({Key? key}) : super(key: key);

  @override
  State<StatefulWidget> createState() => _StaffNewRentOrderScreen();
}

class _StaffNewRentOrderScreen extends State<StaffNewRentOrderScreen> {
  late StaffNewRentOrderBloc _bloc;
  ...
  void _calculateEndDate() {
    if (_rentalDurationController.text.isNotEmpty) {
      try {
        ...
        setState(() {
          _selectedEndDate =
              DateTime.now().add(Duration(days: rentalDuration * 30));
        });
      } catch (e) {
        // Xử lý lỗi nếu không thể parse thành số nguyên
  ...
  void _listener(BuildContext context, StaffNewOrderState state) {
    ...
    if (state is StaffNewOrderGetListProductsSuccessState) {
      ...
      // Filter products to only show rental products ('Thuê') for rent orders
      for (var element in _bloc.listProducts) {
        if (element.product != null && element.orderTypeLabel == 'Thuê') {
                          ...
                          TableCell(
                            ...
                            child: Padding(
                              ...
                              child: TextField(
                                ...
                                maxLines: 1,
                                // Other TextField properties or configurations
                              ),
                            ),
                          ),
                        ],
                              ...
                              onChanged: (text) {
                                if (subSavePointController.text != '') {
                                  var money = text
                                      ...
                                      .trim();
                                  setState(() {
                                    _bloc.subSavePoint = int.tryParse(money) ?? 0;
                                  });
                                }
                              },
                            ),
                        ...
                        children: [
                          ...
                          Container(
                            ...
                            child: TextField(
                              ...
                              onChanged: (text) {
                                if (_vatController.text != '') {
                                  _bloc.vatPercentage =
                                      double.tryParse(text) ?? 0;
                                  _bloc.vatAmount = (_bloc.totalPay *
                                          _bloc.vatPercentage /
                                          100)
                                      .round();
                    ...
                    onTap: () {
                      ...
                      _bloc.add(StaffCreateOrderEvent(
                          ...
                          _monthlyPaymentController.text
                              ...
                              .nonBreaking
                              .trim(),
                          _rentalDurationController.text,
  ...
  // Future<void> _selectRentDate(BuildContext context) async {
  ...
  Future _selectDate(int i) async {
    DateTime? picked = await showDatePicker(
        ...
        initialDate: DateTime.now().add(const Duration(days: 91)),
