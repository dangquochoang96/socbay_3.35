...
class _ServiceScreenState extends State<StaffServiceScreen> {
  ...
  late LocaNotificationService service;
  ...
  void _listener(BuildContext context, StaffServiceScreenState state) {
    if (state is StaffServiceScreenCheckCustomerSuccessState) {
      ...
      _listProducts.add(OrderModel(
          id: 0, product: MachineModel(id: 0, name: "--Chọn máy--")));
    ...
    if (state is StaffServiceScreenCheckCustomerFailedState) {
      setState(() {
        _addNewCustomer = true;
      });
    }

    if (state is StaffServiceScreenChangeTypeServiceState) {
      _currentSelectedValue = state.typeService;
    }

    if (state is StaffServiceScreenCreateTaskSuccessState) {
    ...
    if (state is StaffServiceScreenCreateTaskFailedState) {
                    ...
                    child: Row(
                      children: [
                        _buildButton(
                            ...
                            isPositive: true,
                            action: () {
                              _onCreateTask(_notificationIndex ?? 0);
                            }),
                        const SizedBox(
                          width: 16,
                        ),
  ...
  Future _onCreateTask(int index) async {
    ...
    var orderDetailId = _listProducts
        ...
        .first
        .id;
    _bloc.add(StaffServiceScreenCreateTaskEvent(
        CreateTaskRequest(
            type: 1,
            name: serviceName,
            ...
            timeEnd: '',
            customerId: _bloc.customerInfor?.id ?? 0,
    ...
    if (_bloc.listTaskModel.isEmpty) {
      return const Center(child: Text('no model'));
    }
    TaskModel orderModel = _bloc.listTaskModel[index];
    await service.showNotification(
        id: 0, title: "Bạn có đơn mới", body: "Mã đơn là:  ${orderModel.id}");
  }
  ...
  void _onSearchCustomer() {
    if (customerController.text.isNotEmpty &&
      ...
      _bloc.add(StaffServiceScreenCheckCustomerEvent(customerController.text));
    }
  }
        ...
        return InputDecorator(
          ...
          child: DropdownButtonHideUnderline(
            child: DropdownButton<String>(
              ...
              isExpanded: true,
              onChanged: (String? newValue) {
                _bloc.add(StaffServiceScreenChangeTypeServiceEvent(newValue!));
              },
          ...
          return AlertDialog(
            ...
            content: Column(
              children: [
                ...
                _buildTextField(
                    controller: _addressTextController, hintText: 'Địa chỉ'),
                _buildTextField(
                  controller: customerController,
  ...
  void onNotificationListener(String? payload) {
    if (payload != null && payload.isNotEmpty) {
      ...
      Navigator.push(
        context,
        MaterialPageRoute(builder: (context) => const MyTaskTab()),
      );
    }
  }
}
