...
class ProductCategoryScreenBloc extends Bloc<ProductCategoryScreenEvent, ProductCategoryScreenState> {
  final ApiRepository apiRepository;
  bool isLoading = false;

  List<ProductCategory> listProductCategoryModel = [];

  ProductCategoryScreenBloc({
    required this.apiRepository
  }) : super(ProductCategoryScreenInitialState()) {
    on<ProductCategoryScreenGetListEvent>(
  ...
  FutureOr<void> _mapGetProductCategoryEventToState(
      ...
      Emitter<ProductCategoryScreenState> emit) async {
    if (isLoading) return;
    isLoading = true;
    emit(ProductCategoryScreenInitialState());
    try{
      var url = Uri.http(AppConfig.instance.values.apiUrl,"/api/product/listCate");
      var res = await http.get(url);
      if (res.statusCode == HttpStatus.ok) {
        var l = Map<String,dynamic>.from(json.decode(res.body));
        List<ProductCategory> newListProductModel = List<ProductCategory>.from(l["data"].map((model)=> ProductCategory.fromJson(model)));
