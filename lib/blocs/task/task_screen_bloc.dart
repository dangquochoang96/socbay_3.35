...
class TaskScreenBloc extends Bloc<TaskScreenEvent, TaskScreenState> {
  ...
  FutureOr<void> _mapGetTaskAvailableEventToState(
      ...
      Emitter<TaskScreenState> emit) async {}

  FutureOr<void> _mapGetTaskEventToState(
      TaskScreenGetTaskEvent event, Emitter<TaskScreenState> emit) async {
    isLoading = true;
    emit(MyTaskScreenInitialState());
    var userAccount = await DbManager.instance.getAccounts();
    try{
      var url = Uri.http(AppConfig.instance.values.apiUrl,"/api/tasks/customer/${userAccount.first.id}",{
        'page':event.isRefresh ? 0 : page
      });
      var res = await http.get(url);
      if (res.statusCode == HttpStatus.ok) {
        var l = Map<String,dynamic>.from(json.decode(res.body));
        List<TaskModel> newlistTaskModel = List<TaskModel>.from(l["data"].map((model)=> TaskModel.fromJson(model)));
        if (event.isRefresh) {
          listTaskModel.clear();
          page = 0;
        }
  ...
  FutureOr<void> _mapGetTaskByDayEventToState(
    ...
    emit(MyTaskScreenInitialState());
    try{
      var url = Uri.http(AppConfig.instance.values.apiUrl,"/api/tasks",{
      ...
      if (res.statusCode == HttpStatus.ok) {
        ...
        if(newlistTaskModel.isNotEmpty){pageTaskByday++;}
  ...
  FutureOr<void> _mapGetTaskAssigedEventToState(
    ...
    emit(MyTaskScreenInitialState());
    try{
      var url = Uri.http(AppConfig.instance.values.apiUrl,"/api/tasks/ton-dong",{
  ...
  Future<void>_mapUpdateTaskEventToState(StaffTaskScreenUpdateTaskDoneEvent event,
      Emitter<TaskScreenState> emit) async{
    isLoading = true;
    emit(MyTaskScreenInitialState());
    Map<String, dynamic> params = {
      ...
      "customer": event.status == '1' || event.status == '2' ? null : App.instance.userApp!.id.toString(),
      "user_id":event.status == '1' || event.status == '2' ? null : App.instance.userApp!.id.toString(),
      "time_star": DateFormat('dd/MM/yyyy HH:mm').format(DateTime.parse(event.timeStart.toString())),
    };
    var url = Uri.http(AppConfig.instance.values.apiUrl, "/api/tasks/editt/${event.taskId.toString()}");
  ...
  Future<void>_mapUpdateTaskEventDayToState(StaffTaskScreenUpdateTaskDayDoneEvent event,
      Emitter<TaskScreenState> emit) async{
    isLoading = true;
    emit(MyTaskScreenInitialState());
  ...
  FutureOr<void> _mapGetTaskDoneEventToState(
      ...
      Emitter<TaskScreenState> emit) async {}
  FutureOr<void> _mapBookingDeleteTaskEventToState(BookingDeleteTaskEvent event, Emitter<TaskScreenState> emit)async {
    isLoading = true;
    emit(MyTaskScreenInitialState());
  ...
  FutureOr<void> _mapBookingDeleteTaskToDayEventToState(BookingDeleteTaskToDayEvent event, Emitter<TaskScreenState> emit)async {
    isLoading = true;
    emit(MyTaskScreenInitialState());
    var url = Uri.http(AppConfig.instance.values.apiUrl,"/api/tasks/xoa",{
      'txt-uid': event.taskId.toString(),
