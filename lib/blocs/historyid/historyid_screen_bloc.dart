...
class HistoryidScreenBloc extends Bloc<HistoryidScreenEvent, HistoryidScreenState> {
  ...
  Future<FutureOr<void>> _mapTabPressEventToState(HistoryidScreenTabPressEvent event, Emitter<HistoryidScreenState> emit) async {
    ...
    if(event.index == 0){
      ...
      try{
        ...
        print("X3");
        print("${AppConfig.instance.values.apiUrl}/api/tasks/customer/${App.instance.userApp?.id.toString()}");
        var res = await http.get(url);
        if (res.statusCode == HttpStatus.ok) {
          var l = Map<String,dynamic>.from(json.decode(res.body));
          lstBooking = List<TaskModel>.from(l["data"].map((model)=> TaskModel.fromJson(model)));
          print("X3");
          print(List<TaskModel>.from(l["data"].map((model)=> TaskModel.fromJson(model))));
        }
      }catch(exception){
      ...
      try{
        var url = Uri.http(AppConfig.instance.values.apiUrl,"/api/user/listProduct/${App.instance.userApp?.id.toString()}");
        var res = await http.get(url);
        if (res.statusCode == HttpStatus.ok) {
          var l = Map<String,dynamic>.from(json.decode(res.body));
  ...
  Future<void> _mapBookingDeleteTaskEventToState(BookingDeleteidTaskEvent event, Emitter<HistoryidScreenState> emit)async {
    isLoading = true;
    // final result =
