...
class OrderManagerBlocBySale extends Bloc<OrderManagerEvent, OrderManagerState> {
  ...
  FutureOr<void> _mapGetListOrderManagerEventToState(
      ...
      Emitter<OrderManagerState> emit) async {
    isLoading = true;
    final url = Uri.http(AppConfig.instance.values.apiUrl,"api/order/order-list-bySale/${App.instance.userApp?.id}",{
      'start':event.start,
      'end':event.end
    });
    print(url);
    ...
    if (res.statusCode == HttpStatus.ok) {
      ...
      staffSalesIncomes = List<StaffSalesIncomeModel>.from(l["data"].map((model)=> StaffSalesIncomeModel.fromJson(model)));
      for (var e in staffSalesIncomes) {
        if(e.status == "2"){
          totalPriceAll = totalPriceAll + double.parse(e.totalPrice??"0");
          totalChietKhauAll = totalChietKhauAll + double.parse(e.totalChietKhau??"0");
          totalTruTichDiem = totalTruTichDiem + int.parse(e.totalTruTichDiem??"0");
        }
        totalOrderAll = totalOrderAll + int.parse(e.totalOrder??"0");
      }
      totalPriceAll = totalPriceAll - totalTruTichDiem * 1000 -totalChietKhauAll;
      staffLstOrders = List<StaffOrderDetailModel>.from(l["lstOrders"].map((model)=> StaffOrderDetailModel.fromJson(model)));
      staffLstOrders.reversed;
      for (var element in staffSalesIncomes) {
        if(element.status == "0") totalDonLapMay = int.parse(element.totalOrder??"0");
        if(element.status == "1") totalDonVeSinh = int.parse(element.totalOrder??"0");
        if(element.status == "2") totalDonThayLoi = int.parse(element.totalOrder??"0");
      }
    }
    isLoading = false;
