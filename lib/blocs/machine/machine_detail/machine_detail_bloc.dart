...
class MachineDetailScreenBloc
  ...
  MachineDetailScreenBloc({required this.args, required this.apiRepository})
  ...
  FutureOr<void> _mapStartedEventToState(
    MachineDetailScreenStartedEvent event,
    Emitter<MachineDetailScreenState> emit,
  ) async {
    isLoading = true;
    order = args['id_oder'];
    emit(MachineDetailScreenInitialState());
    var url = AppConfig.instance.apiUri(ApiEndpoints.userHistory(order.id), {
      'phone': args['id_user'],
    });
    ...
    emit(MachineDetailScreenInitialState());
    emit(
      MachineDetailScreenLoadedState(
        ordersModel: ordersModel,
        orderRentModel: orderRentModel,
      ),
    );
  }
}
