...
class CoreReplacementServiceBloc
  ...
  FutureOr<void> _getStartEventToState(CoreReplatementServiceStartEvent event,
      Emitter<CoreReplatementServiceState> emit) async {
    var orderDetail = args["orderDetail"] as OrderDetailModel;
    var url = Uri.http(AppConfig.instance.values.apiUrl,
        "/api/user/detailHistory/${orderDetail.id}");
    ...
    if (res.statusCode == HttpStatus.ok) {
      ...
      try {
        ...
        if (orderDetailModel != null &&
          ...
          final groupedCores = groupBy(orderDetailModel!.orderFilterCoresModel!,
  ...
  FutureOr<void> _updatePaymentStatus(OrderPaymentStatusUpdatedEvent event,
      Emitter<CoreReplatementServiceState> emit) async {
    var orderDetail = args["orderDetail"] as OrderDetailModel;
    var url = Uri.http(AppConfig.instance.values.apiUrl,
        "/api/user/updatePaymentStatus/${orderDetail.id}");
    try {
      var response = await http.post(url, body: {'payment_status': '1'});
      if (response.statusCode == HttpStatus.ok) {
        orderDetailModel = OrderDetailModel.fromJson(
            {...orderDetailModel!.toJson(), 'payment_status': '1'});
        emit(CoreReplatementServiceInitialState());
      } else {
  ...
  FutureOr<void> _createOrUpdateFeedbackTaskProcessed(
      ...
      Emitter<CoreReplatementServiceState> emitter) async {
    //do something
    rating = event.rating;
    des = event.des;
    var orderDetail = args["orderDetail"] as OrderDetailModel;
    var url = Uri.http(
        ...
        "/api/user/rate/${App.instance.userApp?.id}/${orderDetail.id}",
