...
class AccountInfoBloc extends Bloc<AccountInfoEvent, AccountInfoState> {
  ...
  FutureOr<void>_mapGetInfoState(AccountInfoInitEvent event,Emitter<AccountInfoState> emit) async{
    isLoading = true;
    if(App.instance.userApp == null){
      final database =await $FloorAppDatabase.databaseBuilder('socbay.db').build();
  ...
  FutureOr<void> _mapUpdateUserEventToState(
      AccountInfoUpdateUserEvent event, Emitter<AccountInfoState> emit) async {
    isLoading = true;
    ...
    try{
      ...
      final database =await $FloorAppDatabase.databaseBuilder('socbay.db').build();
      final resUpdateUserInfo =
      await apiRepository.updateUserInfo(userInfoRequest);
      if (resUpdateUserInfo.data != null &&
          resUpdateUserInfo.status == 1) {
        ...
        emit(AccountInfoUpdateDoneState(
            isSuccess: false,
  ...
  Future<void> _mapUploadImageEventToState(UploadImageEvent event,
      Emitter<AccountInfoState> emit) async {
    isLoading = true;
    try{
      emit(AccountInfoInitialState());
