...
class ProductScreenBloc extends Bloc<ProductScreenEvent, ProductScreenState> {
  ...
  ProductScreenBloc({
    required this.apiRepository, required this.args
  ...
  FutureOr<void> _mapGetProductCategoryEventToState(
      ...
      Emitter<ProductScreenState> emit) async {
    // if (isLoading) return;
    isLoading = true;
    emit(ProductScreenInitialState());
    var categoryId = args.isNotEmpty ? args['cateId'] : null;
    try{
      var url = Uri.http(AppConfig.instance.values.apiUrl,"/api/product/search",{
      ...
      if (res.statusCode == HttpStatus.ok) {
        ...
        List<ProductModel> newListProductModel = List<ProductModel>.from(l["data"]["data"].map((model)=> ProductModel.fromJson(model)));
