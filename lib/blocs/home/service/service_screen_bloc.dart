...
class ServiceScreenBloc extends Bloc<ServiceScreenEvent, ServiceScreenState> {
  ...
  Future _mapCreateTaskEventToState(
    ...
    try{
      ...
      Map<String, dynamic> params = {
        ...
        "user_create":"161",
        "product_id":event.createTaskRequest.productId==0?null:event.createTaskRequest.productId,
        "images": images,
        //new
      ...
      try{
        ...
        if (response.statusCode == HttpStatus.ok) {
          ...
          if(l["code"] == 1){
            ...
            emit(ServiceScreenCreateTaskSuccessState(
  ...
  FutureOr<void> _mapGetListUserAddressEventToState(
      ...
      Emitter<ServiceScreenState> emit) async {
    isLoading = true;
    emit(ServiceScreenInitialState());
    ...
    try{
      ...
      if (res.statusCode == HttpStatus.ok) {
        ...
        listProducts = List<OrderModel>.from(m["listProducts"].map((model)=> OrderModel.fromJson(model)));
  ...
  Future<void> _mapUploadImageEventToState(ServiceScreenUploadImageEvent event,
      Emitter<ServiceScreenState> emit) async {
    isLoading = true;
    try{
      paths.clear();
