...
class EditRentServiceBloc extends Bloc<EditServiceEvent, EditServiceState> {
  ...
  EditRentServiceBloc({required this.apiRepository, required this.args})
  ...
  FutureOr<void> _mapStartedEventToState(
      EditServiceStartedEvent event, Emitter<EditServiceState> emit) async {
    isLoading = true;
    emit(EditServiceInitialState());
    try{
      var url = Uri.http(AppConfig.instance.values.apiUrl,"/api/rent-tasks/${args['id']}");
  ...
  Future<void> _mapGetServicesEventToState(
      EditServiceGetServicesEvent event, Emitter<EditServiceState> emit) async {
    isLoading = true;
    emit(EditServiceInitialState());
    services.clear();
  ...
  Future<void> _mapUploadImageEventToState(
      EditServiceUploadImageEvent event, Emitter<EditServiceState> emit) async {
    isLoading = true;
    try {
      paths.clear();
    // emit(EditServiceInitialState());
  
    var uri = Uri.parse("$protocol${AppConfig.instance.values.apiUrl}/api/order/upload-image");
  ...
  Future<void> _mapUpdateTaskEventToState(
      EditServiceUpdateTaskEvent event, Emitter<EditServiceState> emit) async {
    isLoading = true;
    emit(EditServiceInitialState());
    List<String>? images = event.updateTaskRequest.images;
    // List<int>? products = event.updateTaskRequest.products;
    if(event.updateTaskRequest.customerId == null || event.updateTaskRequest.customerId == 0 || App.instance.userApp == null){
        isLoading = false;
        return;
      }
    ...
    var url = Uri.http(AppConfig.instance.values.apiUrl,"/api/rent-tasks/edit/${args['id']}");
